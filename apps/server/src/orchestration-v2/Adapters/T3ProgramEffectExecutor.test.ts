...
const encodePhaseCallbackAcknowledgement = Schema.encodeSync(
...
describe("T3ProgramEffectExecutor", () => {
        ...
        expect(launches).toEqual([
          {
            ...
            requestId: ProgramAttemptRequestId.make(
              `program-effect:${reviewEffect.effectId}:launch`,
            ),
            threadId: reviewOwnerThreadId,
            programId,
            taskId: effect.identity.phaseId,
  ...
  it.effect("delivers and recovers one exact nonce-bound Phase callback", () =>
    Effect.gen(function* () {
      const programCoordinatorThreadId = ThreadId.make("thread:callback-program-coordinator");
      ...
      const acknowledgementText = encodePhaseCallbackAcknowledgement({
      ...
      const callbackProjection = (): OrchestrationV2ThreadProjection => {
      ...
      const observed = yield* restarted.observe(
        callbackEffect as unknown as ProgramEffect,
        context,
      );
      expect(Option.isSome(observed)).toBe(true);
      expect(sendCount).toBe(1);
