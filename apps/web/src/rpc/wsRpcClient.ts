...
interface ServerConnectRemoteEnvironmentOptions {
  readonly onProgress?: (event: ServerConnectRemoteEnvironmentEvent) => void;
}
...
export interface WsRpcClient {
  ...
  readonly server: {
    ...
    readonly bootstrapSshRepoBinding: RpcUnaryMethod<
      typeof WS_METHODS.serverBootstrapSshRepoBinding
    >;
    readonly connectRemoteEnvironment: (
      input: ServerConnectRemoteEnvironmentInput,
      options?: ServerConnectRemoteEnvironmentOptions,
...
export function createWsRpcClient(transport = new WsTransport()): WsRpcClient {
  return {
    ...
    server: {
      ...
      bootstrapSshRepoBinding: (input) =>
        transport.request((client) => client[WS_METHODS.serverBootstrapSshRepoBinding](input)),
      connectRemoteEnvironment: async (input, options) => {
        let result: ServerConnectRemoteEnvironmentResult | null = null;
        let failureDetail: string | null = null;

        await transport.requestStream(
          (client) => client[WS_METHODS.serverConnectRemoteEnvironment](input),
          (event) => {
            options?.onProgress?.(event);
            if (event._tag === "connect_finished") {
              result = event.result;
              return;
            }
            if (event._tag === "connect_failed") {
        ...
        throw new Error("Remote environment connect stream completed without a final result.");
      },
